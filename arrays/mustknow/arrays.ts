...
let allInfo: any[] = [];  // This is an array, but its elements can be of any type.
let allInfo2: any = [];   // This can be any type of value (here, it's an array, but it could be a string, number, etc.).
...
let constructorArray: number[] = new Array(5).fill(0);
...
let lastElement: number | undefined = constructorArray.pop(); // 3
let lastElement2: number = constructorArray.pop() ?? 0; // Provide a fallback.
...
let petNames: string[] = ["Sammy", "Link", "Izzy"];
...
let spliceArray: number[] = constructorArray.splice(3, constructorArray.length);[ 0, 0, 1, 3 ]
...
let firstCats: string[] = petNames.slice(0, 2);


// Sorting - Converts elements to strings and sorts them lexiographically.
let randomNums: number[] = [4, 3, 2, 11]; // When sorted, 11 should be at the end
...
// indexOf() function
let firstNames: string[] = ["James", "Chloe", "Brian"];
...
let doubledNumbers: number[] = evenNums.map(num => num * 2); // [ 4, 8, 12, 16, 20 ]